...
interface SettingsPanelProps {
  ...
  onTestDiscord: (draftSettings: AppSettings) => Promise<{ success: boolean; message: string }>;
  onTestWhatsApp: (draftSettings: AppSettings) => Promise<{ success: boolean; message: string }>;
  onReset: () => void;
...
export default function SettingsPanel({
  ...
  const handleTestDiscord = async () => {
    ...
    const result = await onTestDiscord(settings);
    setDiscordTesting(false);
    setDiscordTestResult(result);
    setTimeout(() => setDiscordTestResult(null), 6000);
  };
  ...
  const togglePriority = (prio: string) => {
    ...
    if (current.includes(prio)) {
      setSettings({
        ...
        whatsapp: {
          ...settings.whatsapp,
          priorities: current.filter(p => p !== prio)
        }
      });
    } else {
      setSettings({
  ...
  const togglePlatform = (plat: string) => {
    const current = [...settings.whatsapp.platforms];
    if (current.includes(plat)) {
      setSettings({
        ...settings,
        whatsapp: {
          ...settings.whatsapp,
          platforms: current.filter(p => p !== plat)
        }
      });
    } else {
      setSettings({
    ...
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-4 md:p-6">
      ...
      <div className="flex flex-row lg:flex-col gap-1.5 border-b lg:border-b-0 lg:border-r border-slate-200 dark:border-slate-800 pb-4 lg:pb-0 lg:pr-4 overflow-x-auto whitespace-nowrap">
        <button
          ...
          className={`flex items-center gap-2.5 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all ${activeTab === 'discord'
        ...
        <button
          ...
          className={`flex items-center gap-2.5 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all ${activeTab === 'whatsapp'
            ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-950/40 dark:text-indigo-400'
            : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:bg-slate-800/50'
            }`}
        >
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                Conecte seu bot do Discord para monitorar os webhooks gerados pelas plataformas de venda.
              </p>
            </div>
            ...
            <div className="space-y-3 pt-2">
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-700 dark:text-slate-300">Token do Bot Discord</label>
                <div className="relative">
                  <input
                    ...
                    onChange={(e) => setSettings({
                      ...settings,
                      discord: { ...settings.discord, token: e.target.value }
                    })}
                    placeholder="MTAyNjg4NzkzN... Insira o token do bot gerado no Discord Developer Portal"
                    className="w-full text-xs p-2.5 pr-10 border border-slate-200 dark:border-slate-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-slate-50/50 dark:bg-slate-900/40 dark:text-slate-200"
                  />
                  <button
                    id="btn-show-token"
                    type="button"
                ...
                <p className="text-[11px] text-slate-400 leading-normal">
                  Este bot precisa ter as intents <strong className="font-semibold text-slate-600 dark:text-slate-300">Guild Messages</strong> e <strong className="font-semibold text-slate-600 dark:text-slate-300">Message Content</strong> ativadas em sua aba &quot;Bot&quot; no painel do Discord.
                </p>
              </div>

              {/* Monitor Channels */}
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-700 dark:text-slate-300">IDs dos Canais (separados por vírgula)</label>
                <input
                  id="input-discord-channels"
                  type="text"
                  value={settings.discord.channels}
                  onChange={(e) => setSettings({
                    ...settings,
                    discord: { ...settings.discord, channels: e.target.value }
                  })}
                  placeholder="102758963248..., 10398452145..."
                  className="w-full text-xs p-2.5 border border-slate-200 dark:border-slate-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-slate-50/50 dark:bg-slate-900/40 dark:text-slate-200"
                />
                <p className="text-[11px] text-slate-400 leading-normal">
                  IDs dos canais de texto do Discord onde chegam as notificações das vendas. Se vazio, o bot aceitará mensagens de qualquer canal que possuir acesso.
                </p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Startup history limit */}
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-700 dark:text-slate-300">Limite de Histórico no Startup</label>
                  <input
                    id="input-history-limit"
                    type="number"
                    min="1"
                    max="100"
                    value={settings.discord.historyLimit}
                    onChange={(e) => setSettings({
                      ...settings,
                      discord: { ...settings.discord, historyLimit: parseInt(e.target.value) || 50 }
                    })}
                    className="w-full text-xs p-2.5 border border-slate-200 dark:border-slate-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-slate-50/50 dark:bg-slate-900/40 dark:text-slate-200"
                  />
                  <p className="text-[11px] text-slate-400">Quantidade de mensagens a ler por canal ao ligar (sincronizador de perdas).</p>
                </div>

                {/* Periodic sync interval */}
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-700 dark:text-slate-300">Sincronização Periódica (milissegundos)</label>
                  <input
                    id="input-sync-interval"
                    type="number"
                    step="5000"
                    min="10000"
                    value={settings.discord.syncInterval}
                    onChange={(e) => setSettings({
                      ...settings,
                      discord: { ...settings.discord, syncInterval: parseInt(e.target.value) || 60000 }
                    })}
                    className="w-full text-xs p-2.5 border border-slate-200 dark:border-slate-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-slate-50/50 dark:bg-slate-900/40 dark:text-slate-200"
                  />
                  <p className="text-[11px] text-slate-400">Tempo de busca recorrente para recuperar notificações perdidas (padrão: 60s).</p>
                </div>
              </div>
              ...
              <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-4">
                <div className="flex items-center gap-2 text-xs">
                  <div className={`w-2.5 h-2.5 rounded-full ${systemStatus.discord.connected ? 'bg-emerald-500 animate-pulse' : 'bg-slate-400'}`} />
                  <span className="text-slate-500 dark:text-slate-400">Status atual:</span>
                  <span className="font-semibold text-slate-700 dark:text-slate-200">{systemStatus.discord.statusText}</span>
                </div>

                <button
                  id="btn-test-discord-connection"
                  type="button"
                  onClick={handleTestDiscord}
                  disabled={discordTesting}
                  className="px-3.5 py-1.5 text-xs font-semibold rounded-lg border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 transition-colors inline-flex items-center gap-1.5"
                >
                  {discordTesting ? (
              ...
              {discordTestResult && (
                <div className={`p-3 rounded-lg flex items-start gap-2 text-xs mt-2 ${discordTestResult.success ? 'bg-emerald-50 border border-emerald-150 text-emerald-800 dark:bg-emerald-950/20 dark:border-emerald-900 dark:text-emerald-400' : 'bg-rose-50 border border-rose-150 text-rose-800 dark:bg-rose-950/20 dark:border-rose-900 dark:text-rose-400'}`}>
                  {discordTestResult.success ? <CheckCircle className="w-4 h-4 flex-shrink-0 mt-0.5" /> : <AlertCircle className="w-4 h-4 flex-shrink-0 mt-0.5" />}
        ...
        {activeTab === 'whatsapp' && (
          <div className="space-y-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                Encaminhe de forma imediata notificações importantes do Discord diretamente para o seu WhatsApp.
              </p>
            </div>

            <div className="space-y-4 pt-2">
              {/* Toggle Enable */}
              <label className="flex items-center gap-2.5 p-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/40 cursor-pointer">
                <input
                ...
                />
                <div>
                  <p className="text-xs font-bold text-slate-900 dark:text-slate-100">Ativar Notificações no WhatsApp</p>
                  <p className="text-[10px] text-slate-500">Liga ou desliga o encaminhamento automático de mensagens via WhatsApp Web.</p>
                </div>
              </label>
              ...
              {settings.whatsapp.enabled && (
                <div className="space-y-4 animate-fadeIn">
                  {/* Phone number */}
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-700 dark:text-slate-300">Número do WhatsApp Destinatário (com DDD e DDI)</label>
                    <input
                      ...
                      onChange={(e) => setSettings({
                        ...settings,
                        whatsapp: { ...settings.whatsapp, phoneNumber: e.target.value.replace(/\D/g, '') }
                      })}
                      placeholder="5511999999999"
                      className="w-full text-xs p-2.5 border border-slate-200 dark:border-slate-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-slate-50/50 dark:bg-slate-900/40 dark:text-slate-200"
                    />
                    <p className="text-[11px] text-slate-400">
                      Insira o número completo com DDI (Brasil: 55) + DDD (2 dígitos) + Número. Não inclua espaços, traços ou parênteses. Exemplo: 5511998765432
                    </p>
                  </div>

                  {/* Filter Priorities */}
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-700 dark:text-slate-300 block">Enviar apenas para prioridades:</label>
                    <div className="flex flex-wrap gap-2">
                      {['normal', 'alta', 'urgente'].map((prio) => (
                        <button
                          id={`btn-prio-filter-${prio}`}
                          type="button"
                          key={prio}
                          onClick={() => togglePriority(prio)}
                          className={`text-xs font-semibold px-3 py-1.5 rounded-lg border transition-all ${settings.whatsapp.priorities.includes(prio)
                  ...
                  {/* Filter Platforms */}
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-700 dark:text-slate-300 block">Enviar apenas para plataformas:</label>
                    <div className="flex flex-wrap gap-2">
                      {['ggmax', 'gamemarket', 'desapego'].map((plat) => (
                        <button
                          id={`btn-plat-filter-${plat}`}
                          type="button"
                          key={plat}
                          onClick={() => togglePlatform(plat)}
                          className={`text-xs font-semibold px-3 py-1.5 rounded-lg border transition-all ${settings.whatsapp.platforms.includes(plat)
                            ? 'bg-indigo-600 text-white border-indigo-600 shadow-xs'
                            : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50 dark:bg-slate-800 dark:text-slate-300 dark:border-slate-700'
                            }`}
                        >
                  ...
                  {/* Send synchronized */}
                  <label className="flex items-center gap-2.5 p-3 rounded-xl border border-slate-100 dark:border-slate-800 cursor-pointer">
                    <input
                      id="checkbox-send-recovered"
                      type="checkbox"
                      checked={settings.whatsapp.sendRecovered}
                      onChange={(e) => setSettings({
                        ...settings,
                        whatsapp: { ...settings.whatsapp, sendRecovered: e.target.checked }
                      })}
                      className="rounded text-indigo-600 focus:ring-indigo-500 w-4 h-4"
                    />
                    <div>
                      <p className="text-xs font-bold text-slate-700 dark:text-slate-300">Disparar para mensagens recuperadas do histórico</p>
                      <p className="text-[10px] text-slate-400">Ativa o envio de WhatsApp para as notificações antigas detectadas durante o startup ou sincronização periódica.</p>
                    </div>
                  </label>
                  ...
                  <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-4">
                    <div className="flex items-center gap-2 text-xs">
                      <div className={`w-2.5 h-2.5 rounded-full ${systemStatus.whatsapp.status === 'conectado' ? 'bg-emerald-500 animate-pulse' : 'bg-slate-400'}`} />
                      <span className="text-slate-500 dark:text-slate-400">Status atual:</span>
                      <span className="font-semibold text-slate-700 dark:text-slate-200">{systemStatus.whatsapp.statusText}</span>
                    </div>

                    <button
                      id="btn-test-whatsapp-alert"
                      type="button"
                      onClick={handleTestWhatsApp}
                      disabled={whatsappTesting || systemStatus.whatsapp.status !== 'conectado'}
                      className="px-3.5 py-1.5 text-xs font-semibold rounded-lg border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 disabled:opacity-50 transition-colors inline-flex items-center gap-1.5"
                    >
                      {whatsappTesting ? (
                        <span className="w-3 h-3 border-2 border-indigo-600 border-t-transparent rounded-full animate-spin" />
                      ) : (
                      ...
                      Testar WhatsApp (Mensagem)
                    </button>
                  </div>

                  {whatsappTestResult && (
                    <div className={`p-3 rounded-lg flex items-start gap-2 text-xs mt-2 ${whatsappTestResult.success ? 'bg-emerald-50 border border-emerald-150 text-emerald-800 dark:bg-emerald-950/20 dark:border-emerald-900 dark:text-emerald-400' : 'bg-rose-50 border border-rose-150 text-rose-800 dark:bg-rose-950/20 dark:border-rose-900 dark:text-rose-400'}`}>
                      {whatsappTestResult.success ? <CheckCircle className="w-4 h-4 flex-shrink-0 mt-0.5" /> : <AlertCircle className="w-4 h-4 flex-shrink-0 mt-0.5" />}
                      <span>{whatsappTestResult.message}</span>
                    </div>
                  )}

                  {/* QR Code and Connection Status Block */}
                  <div className="mt-4 p-4 rounded-xl border border-dashed border-slate-200 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-950/20 flex flex-col items-center text-center">
                    <h4 className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-3 flex items-center gap-1.5 self-start">
                      <QrCode className="w-4 h-4 text-emerald-500" />
                      Pareamento WhatsApp Web
                    </h4>

                    {systemStatus.whatsapp.status === 'conectando' && (
                      <div className="py-4 space-y-2">
                        <RefreshCw className="w-8 h-8 text-indigo-500 animate-spin mx-auto" />
                        <p className="text-xs font-semibold text-slate-700 dark:text-slate-300">Inicializando gateway de conexão...</p>
                      </div>
                    )}

                    {systemStatus.whatsapp.status === 'esperando_qr' && (
                      <div className="space-y-3 w-full">
                        {systemStatus.whatsapp.qrCode ? (
                          <div className="p-2.5 bg-white border border-slate-200 rounded-xl max-w-[150px] mx-auto shadow-sm">
                            <img
                              src={systemStatus.whatsapp.qrCode}
                          ...
                          <div className="w-[150px] h-[150px] bg-slate-100 dark:bg-slate-900 rounded-xl flex items-center justify-center mx-auto border border-dashed border-slate-200 dark:border-slate-850">
                        ...
                        <p className="text-[10px] text-slate-500 max-w-sm mx-auto leading-relaxed">
                          Abra o WhatsApp, vá em <strong className="font-semibold">Aparelhos conectados</strong> e aponte a câmera para o QR Code acima.
                        </p>

                        {onTriggerScanSim && (
                          <div className="pt-2">
                            <button
                              id="btn-scan-sim-settings"
                              type="button"
                              onClick={onTriggerScanSim}
                              className="px-4 py-1.5 text-xs font-bold bg-emerald-600 text-white hover:bg-emerald-700 rounded-lg shadow-sm transition-all inline-flex items-center gap-1 w-full justify-center cursor-pointer"
                            >
                              <Zap className="w-3.5 h-3.5" />
                              Simular Escaneamento QR (Testar no Navegador)
                            </button>
                          </div>
                        )}
                      </div>
                    ...
                    {systemStatus.whatsapp.status === 'conectado' && (
                      <div className="py-4 space-y-2">
                        <div className="w-10 h-10 bg-emerald-100 dark:bg-emerald-950/40 rounded-full flex items-center justify-center mx-auto text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-900">
                          <CheckCircle className="w-6 h-6" />
                        </div>
                        <p className="text-xs font-bold text-emerald-600 dark:text-emerald-400">Sessão Ativa & Conectada!</p>
                        <p className="text-[10px] text-slate-500">Seu WhatsApp está pronto para enviar notificações formatadas ao destinatário.</p>

                        <div className="pt-2">
                          <button
                            type="button"
                            onClick={async () => await onDisconnectWhatsApp()}
                            className="px-4 py-1.5 text-[11px] font-bold text-rose-600 bg-rose-50 hover:bg-rose-100 border border-rose-200 dark:border-rose-900/50 dark:text-rose-400 dark:bg-rose-950/20 dark:hover:bg-rose-950/40 rounded-lg shadow-sm transition-all"
                          >
                            Desconectar Celular
                          </button>
                        </div>
